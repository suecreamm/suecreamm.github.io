...
현재 위치가 `/home/hwang`이 맞다면 아래 명령어를 사용해 QE를 다운로드하겠습니다.  wget이나 git 둘 중 하나의 명령어를 쓰면 됩니다. git은 종종 사용이 안 되는 경우가 있는데 wget을 써도 받을 수 있습니다. 이후 `ls` 명령어를 통해 현재 디렉토리에 다운로드가 완료되었는지 확인할 수 있습니다. 이 튜토리얼에서는 git을 사용해 q-e-qe-7.4.1.tar.gz 파일을 다운로드한 경우를 기준으로 진행합니다 (아랫줄 명령어). 
```bash
...
QE의 계산을 위해 입력 파일(예: `ausurf.in`)을 작업 디렉토리로 복사하여 사용하겠습니다. 다음 명령어로 입력 파일을 현재 디렉토리로 복사합니다. *.in 파일은 어떠한 시스템을 계산할 지에 대한 정보를 담고 있습니다. 풀고 싶은 문제를 여기에 정의하게 됩니다.
`mkdir 폴더이름`을 커맨드에 쳐서 계산 데이터를 저장할 폴더를 적당하게 만들고 들어간 상태에서 치면 되는 명령어입니다.
...
{{% callout note %}}
계산이 적절하게 진행되지 않는 경우 ***CRASH***라는 파일이 생성되어 있습니다 (`ls` 명령어로 확인). 이것과 *.out 파일을 열어 문제를 디버깅하면 됩니다.
